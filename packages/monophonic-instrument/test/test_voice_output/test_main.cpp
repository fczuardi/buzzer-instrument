...
void test_voice_output_starts_a_note_with_waveform() {
  ...
  const bool started = output.startNote(60, 261.63f, ToneWaveform::Saw32, 96);

  TEST_ASSERT_TRUE(started);
  TEST_ASSERT_TRUE(output.isPlaying());
  TEST_ASSERT_EQUAL_UINT8(60, output.lastMidiNote);
  TEST_ASSERT_FLOAT_WITHIN(0.01f, 261.63f, output.lastFrequencyHz);
...
void test_voice_output_stops_current_note() {
  ...
  output.startNote(64, 329.63f, ToneWaveform::Square32, 100);
  output.stopNote();
