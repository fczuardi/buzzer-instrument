...
MonophonicNoteAction MonophonicNotePriority::noteOn(
  ...
  const int existingIndex = heldNoteIndex(midiNote);
  if (existingIndex >= 0) {
...
MonophonicNoteAction MonophonicNotePriority::noteOff(uint8_t midiNote) {
  ...
  const bool removingActiveNote = noteActive_ && activeMidiNote_ == midiNote;
  removeHeldNoteAt(static_cast<size_t>(existingIndex));
...
int MonophonicNotePriority::heldNoteIndex(uint8_t midiNote) const {
  for (size_t i = 0; i < heldNoteCount_; ++i) {
    if (heldNotes_[i].midiNote == midiNote) {
